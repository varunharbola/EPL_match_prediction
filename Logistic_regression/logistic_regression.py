...
def plot_prediction(x_test, y_test, theta, title=''):
    ...
    for i in range(len(y_test)):
        h_i = h_logreg(x_test[i,:], theta)
        h.append(h_i)
        if h_i >= 0.5: y_predict.append(1)
    ...
    py.savefig(main_dir+title+'_lin_plot.png')
    py.close()
...
def do_logreg(data, L_i, trained=False, ratio=0.8, save_plot=True,
    ...
    x_train, y_train, x_test, y_test = define_xy(data, L_i, ratio=ratio)
    # import theta1 from theta1.csv if already trained
    if trained:
        theta = np.genfromtxt(main_dir+feature_title+'_theta.csv', delimiter=',')
    else:
        theta = logreg(x_train, y_train, eps=eps, alpha=alpha, lmda=lmda)
        # save theta
        np.savetxt(main_dir+feature_title+'_theta.csv', theta, delimiter=',')
##    theta = np.array([-0.00378961, 0.39707427, -0.41742261])
    # plot a histogram of predictions
    if save_plot:
        plot_prediction(x_test, y_test, theta, title=feature_title)
        for i in range(len(L_i)):
            for j in range(len(L_i)):
                if i != j: plot_decision(x_test, y_test, theta, L_i, i, j, title=feature_title)
...
if __name__ == '__main__':
    ...
    do_logreg(data, range(0,10), trained=True, save_plot=True,
              eps=1e-15, alpha=1e-5, lmda=1e1)
